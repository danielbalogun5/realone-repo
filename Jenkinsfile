pipeline {
    ...
    environment {
        ...
        AWS_ACCOUNT_ID="107439843023"
        AWS_DEFAULT_REGION="us-east-1"
        IMAGE_REPO_NAME="jenkinspipeline"
        IMAGE_TAG= "${env.BUILD_ID}"
        REPOSITORY_URI = "107439843023.dkr.ecr.us-east-1.amazonaws.com/jenkinspipeline"
    }
    stages {
        stage('Git checkout') {
            steps {
         ...
         stage('Logging into AWS ECR') {
                     ...
                     steps {
                       script{
                         ...
                         sh """-s aws ecr get-login-password --region ${AWS_DEFAULT_REGION} |-s docker login --username AWS --password-stdin ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_DEFAULT_REGION}.amazonaws.com"""
                }
